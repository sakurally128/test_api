...
class SWRequests(object):
    def send_requests(self,s,testdata):
        ...
        url = Login().base_url()+testdata["url"]
        # url后面的params参数
        try:
            params = eval(testdata["params"])
            print("请求的参数：%s" %(Login().parm_md5(params['param_token']+params['special_token'])))
    ...
    def wirte_result(self,result, filename="result.xlsx"):
        # 返回结果的行数row_nub
...
if __name__ == "__main__":
    ...
    print(data[0])
    s = requests.session()
    res =SWRequests().send_requests(s,data[1])
    copy_excel("debug_api.xlsx", "result.xlsx")
    SWRequests().wirte_result(res, filename="result.xlsx")